...
let counter' = start<int>(fun n -> printfn "n = %d, waiting..." n)
counter'.Post(1)
counter'.Post(2)
counter'.Post(1)
 ...
 // (mbox:Agent<'T>, cts:CancellationTokenSource) 
       ...
       Agent.Start(fun n ->
         async {
            while true do
               let! msg = n.Receive()
               if i % 20000 = 0 then
                   printfn "agent %d got message '%s' - ThreadID#%d" i msg Thread.CurrentThread.ManagedThreadId } ))
